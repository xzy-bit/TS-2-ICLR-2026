...
class ModelContext:
    ...
    def generate(
        ...
        if isinstance(self.model, LLM):
            ...
            sampling_params = SamplingParams(
                top_k=tf_config.top_k,
                top_p=tf_config.top_p,
                temperature=tf_config.temperature,
                # use_beam_search=True,
                # best_of=5,
                stop_token_ids=stop_token_ids,
                max_tokens=tf_config.max_new_tokens,
                logits_processors=[bad_word_processor] if "deepseek" in tokenizer.name_or_path.lower() else None,
            )
    ...
    def complete(self, config: GenerationConfig, prompts: list[str]) -> Response:
        ...
        if isinstance(self.model, LLM):
            output_strings = self.generate(config, input_ids)
            output_ids = None
        else:
